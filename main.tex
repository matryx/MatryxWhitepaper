...
New discoveries are the result of collaboration between mathematicians, scientists, and engineers, each building on thousands of years of established knowledge.
...
The focus of this whitepaper is to provide a technical description of the \href{https://github.com/matryx/MatryxPlatform}{Matryx Platform} design.
...
In an unexpected turn of events, he declined the prize money, arguing that the contributions of Hamilton and other mathematicians played a significant role in developing his final solution. He declared his ``disagreement with the organized mathematical community"\cite{ritter2010}, arguing that his peers deserved just as much of the award and recognition and that it would be wrong to claim the money and fame for himself. 
...
Previously, Matryx attributed credit to users via manually populated contributor and reference arrays within each Matryx Submission. The commit system is the second iteration of Matryx's attribution model. Because there are still unresolved issues with the commit system as a means of attributing credit to Matryx users, the current primary function of Matryx Commits is to provide indisputable timestamps for original scientific work. Still, Commits accumulate contribution information for multi-commit projects (albeit naively), allowing for a rudimentary approximation of the distribution of work performed over a project's contributing members. The anatomy of Matryx Commits is currently as follows:
...
\item $totalValue$ - The total value generated by this commit and all of its ancestry
...
To create a commit, a user must perform a commit-reveal process (which we've aliased ``claim-create" to avoid confusion with Matryx Commits). This ensures that their content is not front-runnable by malicious actors.
This process entails the user making two transactions:
\begin{enumerate}
\item claimCommit
\item createCommit \textit{or} createSubmission
\end{enumerate}

To claim a commit, the user must provide the commit's hash to the $claimCommit$ function. As mentioned above, the $commitHash$ is the keccak256 hash of the user's address, some secret or salt and the commit's $contentHash$. Once the $claimCommit$ transaction has been mined, the user is free to create their commit by calling the $createCommit$ function with parent commit's $commitHash$ i.e. $parentHash$ (if this commit was a continuation of prior work), their salt, the $contentHash$ of the commit, its $value$, as well as a special isFork flag. When the user sets the isFork flag to true, or creates a commit without a parent, a new group is created for their commit. When a member of a commit's group adds a Matryx user to that group, the new group member is then able to create commits off of any commit created by members of that group, without the transfer of MTX. Conversely, setting isFork to true will assign a new group to the user's commit, allowing the user to work alone or with a new group of their choosing, but will require the user to transfer to the forked commit its $totalValue$ in MTX. After calling $claimCommit$, the user can alternatively call $createSubmission$, which will both create a new Commit as well as a Submission to a Tournament designated by the user.
